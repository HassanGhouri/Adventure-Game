...
public class AdventureGame implements Serializable {
    ...
    private final String[] actionVerbs = {"QUIT","INVENTORY","TAKE","DROP", "CONSUME"}; //List of action verbs (other than motions) that exist in all games. Motion vary depending on the room and game.

    private final FastTravelManager manager = new FastTravelManager();
    ...
    public boolean movePlayer(String direction) {
        ...
        boolean wasVisited = room.getVisited();
        this.player.setCurrentRoom(room);
        //effect players health
        if((!wasVisited && room.getHealthEffect() > 0) || room.getHealthEffect() < 0){
            boolean isDead = this.player.effectHealth(room.getHealthEffect());
            if(isDead){
                return false;
            }
        }
    ...
    public String interpretAction(String command){
        ...
        if (inputArray[0].equals("TRAVEL")){
            int room = Integer.parseInt(inputArray[1]);
            FastTravelCommand travel = new FastTravelCommand(this, this.rooms.get(room));
            manager.execute(travel);
            return null;
        }

        if (inputArray[0].equals("UNDO")){
            manager.undo();
            return null;
        }
        ...
        if (motionTable.optionExists(inputArray[0])) {
            ...
            else if(inputArray[0].equals("CONSUME") && inputArray.length == 2) {
                if(this.player.checkIfObjectInInventory(inputArray[1])) {
                    this.player.consumeObject(inputArray[1]);
                    if(this.player.getHealth() <= 0){
                        return "YOU DIED";
                    }
     ...
     * getVisitedRooms
     * ---------------------------
     * Getter method to get all the rooms the player has visited, excluding forced rooms
     *
     */
    public HashMap<Integer, Room> getVisitedRooms(){
        HashMap<Integer, Room> visited = new HashMap<>();
        for (Room room: this.getRooms().values()){
            if (room.getVisited() && !room.getCommands().contains("FORCED")){
                visited.put(room.getRoomNumber(), room);
            }
        }
        return visited;
    }
     ...
     */
    public FastTravelManager getManager() {return this.manager;}
