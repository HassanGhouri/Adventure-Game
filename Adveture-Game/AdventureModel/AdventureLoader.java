...
public class AdventureLoader {
    ...
    private void parseRooms() throws IOException {
        ...
        while (buff.ready()) {
            ...
            double healthEffect = Double.parseDouble(buff.readLine());
            ...
            Room room = new Room(roomName, roomNumber, roomDescription, healthEffect, adventureName);
    ...
    public void parseObjects() throws IOException {
        ...
        while (buff.ready()) {
            ...
            AdventureObject object = new NonConsumableObject(objectName, objectDescription, location);
            location.addGameObject(object);
        }

        objectFileName = this.adventureName + "/consumables.txt";
        ...
        while (buff.ready()) {
            ...
            String objectDescription = buff.readLine();
            double healthEffect = Double.parseDouble(buff.readLine());
            boolean isTimed =  Boolean.parseBoolean(buff.readLine());
            double timedAffector = Double.parseDouble(buff.readLine());
            String separator = buff.readLine();
            if (separator != null && !separator.isEmpty())
                System.out.println("Formatting Error!");
            int rand = new Random().nextInt(this.game.getRooms().size());
            Room randomRoom = game.getRooms().get(rand);
            while(randomRoom == null || randomRoom.getMotionTable().isForced()){
                rand = new Random().nextInt(this.game.getRooms().size());
                randomRoom = game.getRooms().get(rand);
            }
            AdventureObject object = new ConsumableObject(objectName, objectDescription, randomRoom, healthEffect);
            randomRoom.addGameObject(object);
        }
